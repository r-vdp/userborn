...
impl SubIds {
    pub fn from_dir(directory: &str) -> Self {
        Self {
            uid: SubId::from_file(format!("{directory}/subuid")).unwrap_or_default(),
            gid: SubId::from_file(format!("{directory}/subgid")).unwrap_or_default(),
        }
    }

    pub fn to_dir(&self, directory: &str) -> Result<()> {
        self.uid.to_file(format!("{directory}/subuid"))?;
        self.gid.to_file(format!("{directory}/subgid"))
    }
